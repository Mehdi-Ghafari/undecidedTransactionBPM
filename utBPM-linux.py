...
# region Function FtpUploadTracker
...
# endregion Function FtpUploadTracker
...
def mergeFile(pathEN, pathENT, mergeOUT):
    ...
    with open(mergeOUT, 'w') as new_file:
        new = ''
        for line_a in txt1:
            for line_b in txt2:
...
def createFile(vfnOUT1, folder, pathOUT):
        ...
        with open(pathOUT, 'a',
            ...
            if vSTATUS.strip() == "N":
                ...
                try:
                    ...
                    print(vDIAG + "|" + vPSP_BIN + "|" + vCAPTUREDATE2 + "|" + vAMOUNT)
...
# region Function uploadFile
def uploadFile(vfnOUT2):
    try:
        ftps = FTP_TLS(timeout=15)
        # ftps.set_debuglevel(2)
        ftps.connect('172.30.23.26', 21)
        ftps.login('bpmftp', '4R_UudP#nS')
        ftps.set_pasv(True)
        ftps.prot_p()
        upload_file2 = open(os.path.abspath(vfnOUT2), 'rb')
        print(os.path.abspath(vfnOUT2))
        totalSize = os.path.getsize(os.path.abspath(vfnOUT2))
        print(totalSize)
        print(upload_file2)
        print('/JN-TEST/' + lastFile('/EN'))
        print(vfnOUT2.split('-')[1])

        ftps.cwd('/JN-TEST/' + vfnOUT2.split('-')[1])
        uploadTracker = FtpUploadTracker(int(totalSize))
        ftps.storbinary('STOR {}'.format(lastFile('/EN')), upload_file2, 1024, uploadTracker.handle)
        ftps.quit()
        ftps.close()
        upload_file2.close()
...
# endregion Function uploadFile

# region Function createFTPSOUT
def createFTPSOUT():
    lastFileFTP('/EN')
    lastFileFTP('/EN TARAKONESH')

    print('First')
    vfnEN = 'temp/' + lastFileFTP('/EN')
    print(vfnEN)
    vfnENTRAN = 'temp/' + lastFileFTP('/EN TARAKONESH')
    print(vfnENTRAN)
    ...
    if len(os.listdir(folder)) != 0:
        vfnOUT1 = 'temp/MERGE-python --version' + lastFile('/EN')
        vfnOUT2 = 'OUT/' + lastFile('/EN')
        ...
        if vfnOUT2.split('-')[1] in ftps.nlst():
            ftps2 = FTP_TLS(timeout=15)
            ftps2.set_debuglevel(2)
            ftps2.connect('172.30.23.26', 21)
            ftps2.login('bpmftp', '4R_UudP#nS')
            ftps2.set_pasv(True)
            ftps2.prot_p()
            filelist = []
            ftps2.cwd('/JN-TEST/'+ str(vfnOUT2.split('-')[1]))
            ftps2.retrlines('LIST', filelist.append)
            print("MAHDI.MAHDI")
            print(ftps2.nlst())
            print(str(lastFile('/EN')))
            print(ftps2.cwd('/JN-TEST/'+ str(vfnOUT2.split('-')[1])))
            print("MAHDI.MAHDI")
            if lastFile('/EN') in ftps2.nlst():
                print("File Not Exists")
            else:
                mergeFile(vfnEN, vfnENTRAN, vfnOUT1)
                createFile(vfnOUT1, folder, vfnOUT2)
                uploadFile(vfnOUT2)
        else:
            ftps.cwd('/JN-TEST')
            ftps.mkd(vfnOUT2.split('-')[1])
            createFTPSOUT()
            # uploadFile(vfnOUT2)
    else:
        for filename in os.listdir(folder):
...
# endregion Function createFTPSOUT

# region Function FTPS_Check_directory_exists
def directory_exists(r_dir):
    # in current location
    ftps = FTP_TLS(timeout=15)
    ftps.set_debuglevel(2)
    ftps.connect('172.30.23.26', 21)
...
def ftps_chdir(ftp_connection, dir):
    ...
    if directory_exists(ftp_connection, str(dir)) is False:  # (or negate, whatever you prefer for readability)
...
def lastFile(fn):
    ...
    log = []
    ftps.retrlines('LIST', callback=log.append)
    files = (line.rsplit(None, 1)[1] for line in log)
    arrFilename = list(files)
...
def lastFileFTP(fn):
    ...
    ftps.cwd(fn)
    # arrFilename = ftps.retrlines('NLST')
    ...
    for fn in filenames:
        lf = open('temp/' + fn, "wb")
        ftps.retrbinary("RETR " + fn, lf.write, 8 * 1024)
        lf.flush()
        os.fsync(lf.fileno())
        lf.close()
        return fn
